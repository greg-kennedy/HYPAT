    ...
    def handle(self, *args, **options):
        ...
        for game, releases in sorted(games.items()):
            ...
            try:
                ...
                g = Game(name=game, enabled=True)
                g.save()
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"Failed to import {game}: {type(e).__name__}: {e}"))
                continue
            ...
            for filename, release in sorted(releases.items()):
                ...
                try:
                    ...
                    try:
                        r = Release(game=g,
                            rom=File(file=open("roms/" + filename, 'rb'), name=filename),
                            image=File(file=open("screenshots/" + release.get('image', 'default_snapshot.png'), 'rb'), name=release.get('image', 'default_snapshot.png')),
                            primary=release.get('primary', False),
                            name=release.get('name', ''),
                            alt=release.get('alt', ''),
                            aka=release.get('aka', ''),
                        ...
                        self.stdout.write(self.style.ERROR(f"Failed to import {filename}: {type(e).__name__}: {e}"))

            g.enabled=not allProto
            g.save()

        self.stdout.write(
            self.style.SUCCESS(f"Imported {count} games from file {options['filepath']}")
        )
